...
export function UnifiedDuelInterface({
  questions,
  currentQuestion,
  onAnswer,
  playerAvatar,
  ...
  const handleAnswer = (selectedText: string) => {
    ...
    console.log('🎯 [UNIFIED DUEL] Resposta selecionada:', {
      selectedText,
      correctAnswer: currentQ.correct_answer,
      isCorrect: selectedText === currentQ.correct_answer
    });
    ...
    onAnswer(selectedText); // Enviar diretamente o texto
  };
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            {currentQ.options.map((optionText, index) => {
              ...
              return (
                <motion.button
                  key={`${currentQ.id}-${index}-${optionText}`}
