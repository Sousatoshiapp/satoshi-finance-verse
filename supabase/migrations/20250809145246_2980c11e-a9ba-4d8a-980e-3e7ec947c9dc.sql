-- QUIZ SYSTEM REDESIGN - JANEIRO 2025 (FINAL)
-- Migração corrigida com estrutura de dados adequada
...
CREATE TABLE IF NOT EXISTS public.quiz_sessions (
    ...
    user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE, -- Corrigido: user_id ao invés de created_by
    tournament_id UUID,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- Políticas para sessões (corrigindo para user_id)
DROP POLICY IF EXISTS "Users can view their own quiz sessions" ON public.quiz_sessions;
CREATE POLICY "Users can view their own quiz sessions" ON public.quiz_sessions FOR SELECT USING (user_id IN (SELECT id FROM profiles WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can create their own quiz sessions" ON public.quiz_sessions;
CREATE POLICY "Users can create their own quiz sessions" ON public.quiz_sessions FOR INSERT WITH CHECK (user_id IN (SELECT id FROM profiles WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can update their own quiz sessions" ON public.quiz_sessions;
CREATE POLICY "Users can update their own quiz sessions" ON public.quiz_sessions FOR UPDATE USING (user_id IN (SELECT id FROM profiles WHERE user_id = auth.uid()));
...
-- Políticas para questões das sessões
DROP POLICY IF EXISTS "Users can view quiz session questions" ON public.quiz_session_questions;
CREATE POLICY "Users can view quiz session questions" ON public.quiz_session_questions FOR SELECT USING (
    session_id IN (SELECT id FROM quiz_sessions WHERE user_id IN (SELECT id FROM profiles WHERE user_id = auth.uid()))
);
