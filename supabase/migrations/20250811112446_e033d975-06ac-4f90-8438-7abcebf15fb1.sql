-- Fix create_duel_with_invite function to use correct column names
...
BEGIN
  ...
  FROM (
    ...
    WHERE quiz_topic = invite_record.quiz_topic 
    AND is_approved = true 
    ORDER BY RANDOM() 
    LIMIT 10
  ) q;
  
  -- Create the duel using correct column names for duels table
