-- Fase 2: Gamificação Avançada (com IF NOT EXISTS)
...
-- RLS Policies (só aplicar se as tabelas existirem)
DO $$ 
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'learning_streaks') THEN
        ALTER TABLE public.learning_streaks ENABLE ROW LEVEL SECURITY;
        
        DROP POLICY IF EXISTS "Users can manage their own streaks" ON public.learning_streaks;
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'learning_analytics') THEN
...
-- Aplicar trigger na tabela user_question_progress (se não existir)
DROP TRIGGER IF EXISTS trigger_update_streaks_analytics ON public.user_question_progress;
