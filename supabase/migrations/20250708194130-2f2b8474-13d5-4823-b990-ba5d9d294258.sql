-- Optimize the assign_bot_achievements function to avoid timeouts
...
BEGIN
  -- Process only first 100 bots to avoid timeout
  FOR bot_record IN 
  ...
  LOOP
    ...
    -- Get quiz statistics for this bot (limit to avoid slow queries)
    SELECT 
    ...
    LIMIT 100; -- Limit quiz sessions to check
    
    -- Only check for achievements that make sense for this bot's level
    ...
    -- Only check quiz achievements if bot has quiz sessions
    ...
    -- Commit after every 10 bots to avoid long transactions
