...
CREATE TABLE public.user_question_history (
  ...
  context_type TEXT NOT NULL DEFAULT 'duel', -- 'duel', 'practice', 'battle_royale'
...
-- Constraint para evitar duplicatas (mesmo usuário não pode ver a mesma questão no mesmo contexto no mesmo dia)
CREATE UNIQUE INDEX idx_user_question_daily_unique ON public.user_question_history(user_id, question_id, context_type, DATE(seen_at));
