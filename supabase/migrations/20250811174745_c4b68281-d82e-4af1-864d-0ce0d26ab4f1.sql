-- CRITICAL SECURITY FIX: Prevent stalker monitoring of user activity
...
-- =====================================
-- 6. UPDATE EXISTING POLICIES TO LOG ACCESS
-- =====================================

-- Create trigger function to log privacy-sensitive access
...
AS $$
BEGIN
  -- Log the access attempt (without blocking it)
...
CREATE TRIGGER log_user_presence_access
  AFTER SELECT ON public.user_presence
  FOR EACH ROW EXECUTE FUNCTION public.log_privacy_access();

CREATE TRIGGER log_activity_feed_access
  AFTER SELECT ON public.activity_feed
  FOR EACH ROW EXECUTE FUNCTION public.log_privacy_access();