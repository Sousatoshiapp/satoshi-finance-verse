...
-- Versão com limpeza completa das policies existentes

-- Fase 1: Limpeza TOTAL de todas as policies problemáticas
...
DROP POLICY IF EXISTS "Users can view only their own activity feed" ON public.activity_feed;
DROP POLICY IF EXISTS "Public crypto payments are viewable by everyone" ON public.crypto_payments;
DROP POLICY IF EXISTS "Users can view their own crypto payments only" ON public.crypto_payments;
...
DROP POLICY IF EXISTS "Admin sessions restricted to owners only" ON public.admin_sessions;
DROP POLICY IF EXISTS "Admin tokens accessible only by service role" ON public.admin_tokens;
...
-- Fase 2: Criar views seguras para dados públicos
...
-- Fase 3: Policies MÁXIMA SEGURANÇA nas tabelas principais

-- PROFILES: Apenas usuário dono
...
CREATE POLICY "crypto_payments_secure_select" 
...
CREATE POLICY "crypto_payments_secure_insert" 
ON public.crypto_payments 
FOR INSERT 
...
CREATE POLICY "activity_feed_secure_select" 
...
CREATE POLICY "admin_sessions_secure_select" 
...
-- Fase 5: Função segura para leaderboard
