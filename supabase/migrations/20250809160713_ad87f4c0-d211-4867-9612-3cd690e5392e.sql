-- Dropar todos os triggers e funções dependentes antes do backup
-- Usando CASCADE para remover dependências

DROP TRIGGER IF EXISTS tr_question_hash_sync ON public.quiz_questions CASCADE;
DROP FUNCTION IF EXISTS public.update_question_hash() CASCADE;

-- FASE 1: Backup e limpeza das perguntas antigas (terceira tentativa)
...
SELECT 
    ...
    'system_migration',
    jsonb_build_object(
        'action', 'legacy_questions_backup',
        'total_questions_moved', (SELECT COUNT(*) FROM public.quiz_questions_legacy),
        'migration_date', now(),
        'reason', 'Refatoração completa para novo sistema de categorias'
    )
