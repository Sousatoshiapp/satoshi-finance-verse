-- Update get_dashboard_data_optimized function to include profile_image_url
...
BEGIN
  ...
  SELECT 
    ...
    p.points,
    p.streak,
    p.profile_image_url,  -- Make sure this is included
    p.current_avatar_id,
    p.subscription_tier,
    p.created_at,
    p.updated_at
  ...
  next_level_xp := (COALESCE(profile_data.level, 1) + 1) * 100;
  ...
  result := json_build_object(
    'profile', json_build_object(
      ...
      'profile_image_url', profile_data.profile_image_url,  -- Include profile image
