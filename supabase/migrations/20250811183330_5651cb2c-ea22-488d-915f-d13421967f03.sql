...
-- 1. Corrigir políticas do profiles
...
DROP POLICY IF EXISTS "Public portfolios have limited visibility" ON portfolios;
...
DROP POLICY IF EXISTS "Limited public activity visibility" ON activity_feed;
...
-- 5. Corrigir políticas dos admin_tokens
DROP POLICY IF EXISTS "Service role can manage admin tokens" ON admin_tokens;
DROP POLICY IF EXISTS "Only service role can access admin tokens" ON admin_tokens;
