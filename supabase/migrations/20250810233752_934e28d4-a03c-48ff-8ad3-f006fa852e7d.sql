-- Criar RPC create_duel_with_invite para sincronizar duelos
...
BEGIN
  ...
  -- Gerar perguntas do duelo
  DECLARE
    duel_questions jsonb;
  BEGIN
    -- Buscar 10 perguntas aleatórias do tópico
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', id,
        'question', question,
    ...
    FROM (
      ...
      FROM quiz_questions
      WHERE topic = invite_record.quiz_topic
      AND is_active = true
      ORDER BY RANDOM()
      LIMIT 10
    ) q;
