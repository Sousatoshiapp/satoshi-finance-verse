-- Drop and recreate the get_dashboard_data_optimized function with correct column reference
...
BEGIN
  ...
  -- FIXED: Buscar time do usuário - usar dt.name em vez de dt.team_name
  SELECT jsonb_build_object(
    'id', dt.id,
    'team_name', dt.name,
  ...
  -- Calcular BTZ yield
  btz_yield := (profile_data->>'points')::numeric * 0.001;
